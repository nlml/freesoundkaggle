# from https://raw.githubusercontent.com/zcaceres/spec_augment/master/exp/nb_SparseImageWarp.py
...
def augment_spectrogram(im, RANDOM_VOLUME, SPEC_AUGMENT_PROB, repeat=3):
    im = im[:1]
    for f in [time_warp, freq_mask, time_mask]:
        if np.random.rand() < SPEC_AUGMENT_PROB:
            im = f(im)
    if RANDOM_VOLUME:
        im *= (torch.rand(1) * RANDOM_VOLUME + (RANDOM_VOLUME * 2 - 1))
    if repeat > 1:
        return im.repeat([repeat, 1, 1])
    return im
...
def time_warp(spec, W=5):
    ...
    src_pts, dest_pts = torch.tensor([[[y, point_to_warp]]]), torch.tensor([
        [[y, point_to_warp + dist_to_warp]]])
...
def time_mask(spec, T=30, num_masks=1, replace_with_zero=False):
    cloned = spec.clone()
